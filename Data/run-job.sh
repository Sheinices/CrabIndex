#!/usr/bin/env bash
# Run one CrabIndex HTTP job (crontab or manual).
# Usage: run-job.sh <name> <url> <max_time>
# CrabIndex returns ok / work / disabled. Long jobs may start work in the
...
LOCK_DIR="${LOCK_DIR:-/tmp/crabindex-cron-locks}"
