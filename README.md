...
Нагрузка сильно зависит от режима базы:

| Сценарий | Где запускать | Режим базы |
| --- | --- | --- |
| Личный поиск дома (Prisma, Lampa) | ПК, ноутбук или мини-ПК с SSD | Синхронизация с `sync.crab.rip` |
| Свой сервер для нескольких пользователей | VPS или домашний сервер с SSD | Синхронизация |
| Своя база, парсинг трекеров | VPS с SSD / NVMe, 2+ ядра и 4+ ГБ (лучше 4 и 8) | Собственный парсинг + FlareSolverr |
| Сервер синхронизации для других | VPS или выделенный сервер с NVMe, 4+ ядер, 8-16 ГБ | Собственный парсинг + FlareSolverr |

Почему не стоит парсить на домашнем компьютере:

- база - около 1,3 млн мелких файлов (5-6 ГБ), каждое изменение раздачи переписывает её файл;
- индекс `masterDb.bz` (около 60 МБ) сохраняется целиком каждые 5 минут, если база менялась: при постоянном парсинге до 15-17 ГБ записи в сутки;
- пересчёт статистики раз в 90 минут перечитывает всю базу;
- журнал изменений (`logFdb`) пишет строку на каждую раздачу и на долго работающем сервере вырастает до гигабайт; с 1.0.5 он выключен по умолчанию и ограничен 1 ГБ, в старых установках проверьте `init.yaml` или раздел **Логи** админ-панели;
- FlareSolverr держит в памяти браузер Chrome и в пиках занимает 1,5-2 ядра.

HDD и сетевые диски (SMB, NFS) не подходят: на миллионе мелких файлов всё начинает тормозить. Дома ставьте в режиме синхронизации (`install.sh --db sync`), выключите `logFdb`, увеличьте `timeStatsUpdate` и исключите папку CrabIndex из антивируса, облачной синхронизации, индексации и бэкапов. Подробно, с настройками: [docs/docs/deployment/where-to-run.md](docs/docs/deployment/where-to-run.md).
...
| Блокировка доменов | Запросы со сторонних сайтов (заголовки `Origin` / `Referer`): встроенный список из 19 доменов, который нельзя изменить из панели, плюс свои чёрный и белый списки и режим «только разрешённые домены» |
