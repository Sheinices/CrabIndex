...
Основная и проверенная платформа CrabIndex - Linux (systemd + cron) и Docker. Код сервера при этом кроссплатформенный: платформо-зависимая часть одна - обработка сигналов остановки. Для Windows и macOS с каждым релизом выпускаются готовые архивы (`crabindex-windows-x86_64.zip`, `crabindex-macos-arm64.tar.gz`, `crabindex-macos-x86_64.tar.gz`) - пошаговая установка из них описана в разделе [Установка](../installation.md#macos) ([Windows](../installation.md#windows)). Сервер также собирается из исходников обычным `cargo build`. Готового установщика и службы «из коробки» нет.
