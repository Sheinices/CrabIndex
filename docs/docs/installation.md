...
:::tip[Где запускать]
Собственный парсинг постоянно пишет на диск (база - больше миллиона мелких файлов) и с FlareSolverr требует 2+ ядер и 4+ ГБ. Для домашнего компьютера выбирайте режим синхронизации, для своей базы и сервера синхронизации - VPS с SSD. HDD и сетевые диски не подходят. Подробно: [Где запускать](deployment/where-to-run.md).
:::
...
4. Ответьте на вопросы. Сначала установщик спросит язык интерфейса (по умолчанию - по локали системы: `ru*` - русский, иначе английский); задать его сразу можно флагом `--lang ru|en` или переменной `CRABINDEX_LANG`. Затем путь админ-панели:
   ...
   Своё название - один сегмент из строчных латинских букв, цифр, `-` и `_`, длиной 2-32 символа (например, `/my-panel`). Служебные пути (`/api`, `/cron`, `/stats`, `/docs` и другие) заняты. Если порт `9117` занят, установщик спросит порт. Затем он спросит [источник базы](#db-source): синхронизация с `sync.crab.rip` (рекомендуется) или собственный парсинг трекеров. Только при собственном парсинге и без `--flaresolverr` / `--no-flaresolverr` установщик спросит, нужен ли обход Cloudflare.
...
### Источник базы {#db-source}

При новой установке (когда `init.yaml` создаётся из шаблона) установщик спрашивает, откуда брать базу раздач:

```text
Источник базы раздач:
  1) Синхронизация с сервером sync.crab.rip (рекомендуется): готовая база скачивается и обновляется сама, минимум нагрузки
  2) Собственный парсинг трекеров: база наполняется с нуля, нужны ресурсы и для части трекеров FlareSolverr
Выберите [1]:
```

- **синхронизация (`--db sync`)** - в `init.yaml` задаётся `syncapi: https://sync.crab.rip` (если там уже указан свой сервер синхронизации, он сохраняется), список `synctrackers` не меняется. В crontab ставятся только служебные задания (`/jsondb/...` и `/cron/maintenance/...`), задания парсинга трекеров и `/cron/cloudflare/...` не ставятся. FlareSolverr не нужен: установщик о нём не спрашивает и выключает обход Cloudflare, если не передан `--flaresolverr`;
- **собственный парсинг (`--db parse`)** - в `init.yaml` задаётся `syncapi: ""`, ставится полный `Data/crontab`, затем установщик спрашивает про FlareSolverr. Полная база наполняется несколько дней. Трекеры за Cloudflare (rutracker, kinozal, selezen и другие) парсятся только через FlareSolverr: он запускает настоящий браузер Chrome и требует минимум 2 ядра CPU и 4 ГБ памяти (рекомендуется 4 ядра / 8 ГБ).

На слабом сервере (меньше 2 ядер или 4 ГБ памяти) собственный парсинг не рекомендуется: при выборе `parse` установщик предупреждает и спрашивает подтверждение, при отказе остаётся синхронизация. С `--yes` он только предупреждает. Без терминала и с `--yes` по умолчанию выбирается синхронизация.

Источник виден в финальном сообщении (строка «База»). Перейти на другой режим позже: `... | sudo bash -s -- --update --db parse` (или `--db sync`). При `--update` без `--db` установщик ничего не спрашивает и сохраняет текущие `syncapi` и набор заданий crontab.
...
| `--lang ru\|en`, `--lang=ru` | Язык интерфейса установщика. Без флага берётся `CRABINDEX_LANG=ru\|en`, иначе установщик спрашивает язык при запуске; без терминала или с `--yes` - по локали системы (`ru*` - русский, иначе английский) |
| `--db sync\|parse`, `--db=sync` | [Источник базы](#db-source): `sync` - синхронизация с `sync.crab.rip` и crontab без заданий парсинга трекеров (по умолчанию без вопроса), `parse` - собственный парсинг с полным crontab и вопросом про FlareSolverr. Без флага берётся `CRABINDEX_DB=sync\|parse`, иначе на новой установке установщик спрашивает. С `--update` переключает существующую установку |
...
- **собственный парсинг** - установите `Data/crontab` (установщик делает это сам в режиме `--db parse`, см. [Источник базы](#db-source));
