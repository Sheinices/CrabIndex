...
:::note[FlareSolverr]
Обход Cloudflare (`--flaresolverr`) требует заметно больше ресурсов, чем сам CrabIndex: минимум 2 ядра и 4 ГБ памяти, рекомендуется 4 ядра и 8 ГБ. На слабом сервере ставьте CrabIndex без него. Подробно: [требования к серверу](configuration/flaresolverr.md#требования-к-серверу).
:::
