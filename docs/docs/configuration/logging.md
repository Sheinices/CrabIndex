...
```yaml
logFdb: false             # журнал изменений базы; по умолчанию выключен
logFdbRetentionDays: 7    # удалять файлы старше N дней; 0 - не удалять
logFdbMaxSizeMb: 1024     # общий лимит размера fdb-журналов, МБ; 0 - без лимита
...
`Data/log/fdb.*.log` - строка JSON «было / стало» на каждое добавление и изменение раздачи. Он нужен для разбора проблем («почему у раздачи поменялся magnet»), для работы поиска и синхронизации не нужен. При активном парсинге пишет на диск гигабайты (на одном сервере журналы заняли 11 ГБ), поэтому с версии 1.0.5 он **по умолчанию выключен**, а общий размер ограничен 1024 МБ. В установках до 1.0.5 в `init.yaml` обычно явно стоит `logFdb: true` - выключите его, если журнал не нужен.

Управлять удобнее в админ-панели: **Логи → Журнал изменений базы**. Там переключатель, текущий размер журналов на диске, срок хранения и лимит размера, кнопка «Удалить журналы». Изменения сохраняются в конфиг, как в разделе **Настройки**. Срок и размер проверяются раз в минуту: самые старые файлы удаляются автоматически.
