...
import { BookOpen, Braces, Clock, Database, ExternalLink, GitCommit, HardDrive, RefreshCw, Server, Shield } from 'lucide-react'
...
import { formatDate, formatDuration, formatNumber, formatRelative, jobPercent } from '../lib/format.js'
...
export function JobList({ jobs, empty = 'Нет активных задач' }) {
      ...
      {jobs.map((job) => {
        ...
        return (
          <li key={job.id || `${job.tracker}:${job.job}`}>
            ...
            {job.summary ? <p className="mt-1 text-xs text-muted">{job.summary}</p> : null}
          </li>
        )
      })}
    </ul>
...
export function WafCard() {
  const { data, error, loading } = usePolling(() => getWafOverview('60m'), 10_000)
  const t = data?.totals || {}
  const pct = t.requests > 0 ? Math.round((t.blocked / t.requests) * 1000) / 10 : 0
  return (
    <section className="card p-5" aria-labelledby="ov-waf">
      ...
      {loading && !data ? (
        ...
        <p className="text-sm text-muted">Статистика недоступна: {error.message}</p>
      ) : data?.enabled === false ? (
        <p className="text-sm">
          <StatusDot tone="muted" label="WAF выключен" />{' '}
          <Link to="/settings?group=waf" className="text-accent hover:underline">
            Включить
          </Link>
        </p>
      ) : (
        <dl className="grid grid-cols-2 gap-3">
          <div>
            ...
            <dd className="text-xl font-semibold tabular-nums">{formatNumber(t.requests)}</dd>
          </div>
          <div>
            <dt className="text-xs text-muted">Заблокировано</dt>
            <dd className={`text-xl font-semibold tabular-nums ${t.blocked ? 'text-danger' : ''}`}>
              {formatNumber(t.blocked)} <span className="text-xs font-normal text-muted">{pct}%</span>
            </dd>
          </div>
        </dl>
      )}
...
export function OverviewPage() {
  const { data, error, loading, reload } = usePolling(() => getOverview(), 10_000)
  const o = data || {}
    ...
    <>
      ...
      {loading && !data ? (
        ...
        <div className="space-y-6">
          <section aria-label="Показатели" className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <Stat icon={GitCommit} label="Версия" value={o.version || '-'} hint={[o.gitSha, o.buildDate && formatDate(o.buildDate)].filter(Boolean).join(' · ')} />
            <Stat icon={Clock} label="Аптайм" value={formatDuration(o.uptimeSeconds)} hint={o.listen ? `Слушает ${o.listen}` : undefined} />
            <Stat icon={Database} label="Раздачи" value={formatNumber(o.torrents)} hint={`${formatNumber(o.masterDbKeys)} ключей · fast ${formatNumber(o.fastDbKeys)}`} />
            <Stat icon={HardDrive} label="Обновление БД" value={formatRelative(o.lastUpdateDb)} hint={formatDate(o.lastUpdateDb)} />
          </section>

          <div className="grid gap-6 lg:grid-cols-3">
            <section className="card p-5 lg:col-span-2" aria-labelledby="ov-jobs">
              ...
              <section className="card p-5" aria-labelledby="ov-sync">
                ...
                <dl className="space-y-2 text-sm">
                  <div className="flex justify-between gap-3">
                    ...
                    <dd>
                      <StatusDot tone={sync.enabled ? 'ok' : 'muted'} label={sync.enabled ? 'включена' : 'выключена'} />
                    </dd>
                  </div>
                  <div className="flex justify-between gap-3">
                    <dt className="text-muted">Источник</dt>
                    <dd className="truncate font-mono text-xs">{sync.syncapi || '-'}</dd>
                  </div>
                  <div className="flex justify-between gap-3">
              ...
              <section className="card p-5" aria-labelledby="ov-links">
                ...
                <ul className="space-y-1 text-sm">
                  {[
                    ...
                    <li key={href}>
                      <a href={href} target="_blank" rel="noopener" className="flex items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-surface-2">
